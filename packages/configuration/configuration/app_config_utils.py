...
def app_config_get_environment_id(
    appconfig: Any, app_id: str, env_name: str, create_if_not_exists: bool = False
) -> str:
    return _get_or_create_id(
        env_name,
        (
...
def app_config_create_hosted_configuration_version(
    appconfig: Any, app_id: str, config_profile_id: str, configuration: dict[str, ConfigurationSection]
) -> int:
...
def app_config_data_get_latest_configuration(
    appconfigdata: Any, app_id: str, env_id: str, config_profile_id: str
) -> dict[str, Any]:
    initial_configuration_token = appconfigdata.start_configuration_session(
        ApplicationIdentifier=app_id, EnvironmentIdentifier=env_id, ConfigurationProfileIdentifier=config_profile_id
    ).get('InitialConfigurationToken')

    # For now we assume we're getting the latest configuration is one round
    response = appconfigdata.get_latest_configuration(ConfigurationToken=initial_configuration_token)
...
def app_config_deploy_service_configuration(
    service_name: str,
    platform: Platform,
    stage: Stage,
    region: str,
    ...
    try:
        with open(config_file) as file:
            ...
            logger.info(
                f'Starting configuration deployment for app: {app_name}, config name: {config_name}, app id: {app_id}, profile: {config_profile_id}, version: {version_number}'
            )  # noqa: E501
...
def _get_or_create_id(
    name: str, list_func: Callable[[str], dict], create_func: Callable[[], dict] | None = None
) -> str:
