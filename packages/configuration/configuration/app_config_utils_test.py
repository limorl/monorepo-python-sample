import pytest
from configuration.app_config_utils import get_app_name, get_config_name, get_default_configuration_html_heading_name, DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_DEV, DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_STAGING, DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_PROD, app_config_get_application_id, app_config_get_environment_id, app_config_get_html_heading_id, app_config_data_get_latest_configuration, app_config_get_profile_id, app_config_create_hosted_configuration_version
from environment.service_environment import Platform, Stage


def test_get_app_name():
    assert get_app_name('test') == 'services/test'
...
def test_get_default_configuration_html_heading_name():
    assert get_default_configuration_html_heading_name(Stage.DEV) == DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_DEV
    assert get_default_configuration_html_heading_name(Stage.STAGING) == DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_STAGING
    assert get_default_configuration_html_heading_name(Stage.PROD) == DEFAULT_CONFIGURATION_DEPLOYMENT_STRATEGY_PROD
...
def test_pp_config_get_html_heading_id_stratey_exists(mock_appconfig, mock_list_deployment_strategies_response):
    ...
    strategy_id = app_config_get_html_heading_id(mock_appconfig, strategy_name)
...
def test_app_config_get_html_heading_id_does_not_exist_expected_error(mock_appconfig, mock_list_deployment_strategies_response):
    strategy_name = mock_list_deployment_strategies_response['Items'][0]['Name']
    mock_list_deployment_strategies_response['Items'] = []
    mock_appconfig.list_deployment_strategies.return_value = mock_list_deployment_strategies_response

    with pytest.raises(KeyError) as err:
        app_config_get_html_heading_id(mock_appconfig, strategy_name)
